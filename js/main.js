...
//  `email` and makes those available as attributes. The `constructor()`
//  username will split before @ symbol on the 'email' value
...
// Class created  that extends the `Person` class called `Student`.
// `this.attendance` is set to an empty Array (`[ ]`). 
//'attendance' property will be used to record and track attendance. 
...
class Course {
    ...
    //  `addStudent()` prompts the user for information required to create a 
    // new `Student` object (`name`, `email`)
    addStudent(){
     ...
     // Prompt `setTeacher()`  for the user information required
    // to create a `Teacher` object (`name`, `email`) and
...
setTeacher(){
    ...
    //  Attendance record set `markAttendance()` method is to retreive the 'Student' object
    //  out of the `this.students` Array. 
    // The default behavior is when student is marked present.
    // The alternate behavior is when the student is marked absent'
...
let courseTitle = prompt ('Enter Course Title (e.g.Intro to JavaScript):');
// TODO: Prompt the user for the  `courseDescription` (the descriptive summary of the course).
let courseDescription = prompt (' Enter Course Description', 'A great course for everyone');
